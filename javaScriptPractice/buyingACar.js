...
function nbMonths(startPriceOld, startPriceNew, savingperMonth, percentLossByMonth){
    if (startPriceOld === startPriceNew) {
      ...
      let savings = 0;
      let months = 0;
      while (savings <= startPriceNew-startPriceOld) {
          months++;
        if (months%2 == 0) {
            percentLossByMonth = percentLossByMonth + 0.005;
        }
          savings = savings + savingperMonth;
        startPriceOld = startPriceOld * (1-percentLossByMonth);
        startPriceNew = startPriceNew * (1-percentLossByMonth);
        console.log(savings-(startPriceNew-startPriceOld));
      }
